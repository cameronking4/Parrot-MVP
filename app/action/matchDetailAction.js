
import ApiUtility,{fetchPost,fetchAuthPost} from '../reducer/lib/ApiUtility';
import AuthUtility from '../reducer/lib/AuthUtility';

import Config from '../Config';

import { 
	
	FETCH_MATCHDETAIL_SUCCESS,
	FETCH_MATCHDETAIL_REQUEST,
	FETCH_MATCHDETAIL_ERROR,

  // FETCH_MATCHDETAILFAVUPDATE_REQUEST,
  // FETCH_MATCHDETAILFAVUPDATE_SUCCESS,
  // FETCH_MATCHETAILFAVUPDATE_ERROR,

  // FETCH_MATCHDETAILBOOKMARKUPDATE_SUCCESS,
  // FETCH_MATCHDETAILBOOKMARKUPDATE_ERROR,
  // FETCH_MATCHDETAILBOOKMARKUPDATE_REQUEST,


 } from '../constants/action-types';

export function fetchMatchDetail(path,bundle){
  return function(dispatch){
    dispatch({ 
    			type: FETCH_MATCHDETAIL_REQUEST,
    			payload: { isLoading: true },
          
    });
    
    console.log('action:fetch-detail::',path,bundle);
    
            fetchAuthPost(path,bundle,function(response){

              // alert('response'+JSON.stringify(response));
              console.log('action:PostDetailResponse',response);
              if(response.success){

                      return  dispatch({
                            type: FETCH_MATCHDETAIL_SUCCESS,
                            payload:  response.data.data[0]

                      }); 

              }
              else{
                return  dispatch({
                        type: FETCH_MATCHDETAIL_ERROR,
                        payload: null
                      }); 
              }

            },function(error){
                return  dispatch({
                        type: FETCH_MATCHDETAIL_ERROR,
                        payload: null
                    }); 
            });
  }
}



// export function updateDetailForFavorite(response){
//   return function(dispatch){
//     dispatch({ 
//           type: FETCH_MATCHDETAILFAVUPDATE_REQUEST,
//           payload: { isLoading: true },
//     });
    
//     //console.log('action:UPDATE -UPDATEPackage-list::',path,bundle);
    

//       var that =this;
            
//         fetchAuthPost( Config.API_URL+'/post/listing',{_id:response.data.post},function(response){


//               //console.log('action:UPDATE:PackageResponse',response.data.data[0]);
//               if(response.success && response.data.data.length > 0){
//                   // alert(JSON.stringify(response))
//                       return  dispatch({
//                             type: FETCH_MATCHDETAILFAVUPDATE_SUCCESS,
//                             payload:  response.data.data[0] 
//                       }); 

//               }
//               else{
//                 return  dispatch({
//                         type: FETCH_MATCHDETAILFAVUPDATE_ERROR,
//                         payload: null
//                       }); 
//               }

//             },function(error){
//                 return  dispatch({
//                         type: FETCH_MATCHDETAILFAVUPDATE_ERROR,
//                         payload: null
//                     }); 
//                 // alert('ERRORRRRR');

//             });
//   }        
// }


// export function updateDetailForBookmark(response){
//   return function(dispatch){
//     dispatch({ 
//           type: FETCH_MATCHDETAILBOOKMARKUPDATE_REQUEST,
//           payload: { isLoading: true },
//     });
    
//     //console.log('action:UPDATE -UPDATEPackage-list::',path,bundle);
    

//       var that =this;
            
//         fetchAuthPost( Config.API_URL+'/post/listing',{_id:response.data.post},function(response){


//               //console.log('action:UPDATE:PackageResponse',response.data.data[0]);
//               if(response.success && response.data.data.length > 0){
//                   // alert(JSON.stringify(response))
//                       return  dispatch({
//                             type: FETCH_MATCHDETAILBOOKMARKUPDATE_SUCCESS,
//                             payload:  response.data.data[0] 
//                       }); 

//               }
//               else{
//                 return  dispatch({
//                         type: FETCH_MATCHDETAILBOOKMARKUPDATE_ERROR,
//                         payload: null
//                       }); 
//               }

//             },function(error){
//                 return  dispatch({
//                         type: FETCH_MATCHDETAILBOOKMARKUPDATE_ERROR,
//                         payload: null
//                     }); 
//                 // alert('ERRORRRRR');

//             });
//   }        
// }


