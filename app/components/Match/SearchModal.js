...
getSearch(q){
    if(q== ' '){
      ...
      this.props.userAction.searchUserList(path,bunch);
    }
}
...
goToDetail(item){
  // alert(JSON.stringify(item))
    ...
    this.props.oncloseModal('goToDetail',item);
}
  ...
  return(
      ...
      <View style={[styles.container,{flexDirection:'column'}]}>
        ...
        <TouchableOpacity onPress={()=>this.goToDetail(item)} style={[styles.container]} >
          ...
          <View>
              <Text numberOfLines={1} style={{width:Dimensions.get('window').width-140,fontSize:19,color:'#000',fontWeight:'400'}}>{item.item.username}</Text>
              <Text numberOfLines={1} style={{width:Dimensions.get('window').width-140,fontSize:19}}>{item.item.firstname}</Text>
              <Text numberOfLines={1} style={{width:Dimensions.get('window').width-140,fontSize:19}}>{item.item.lastname}</Text>
          </View>
...
// alert(JSON.stringify(this.props.user.userList));
    if(this.props.user.isLoading){
      return this.spinner();
    }

    else if(this.props.user.userList == null){
     ...
     return(
        ...
        <View style={{height:Screen.height-80,justifyContent:'center',}} >
          ...
          <View style={{justifyContent:'flex-start',alignItems:'center'}} >
            ...
            <Text  style={{fontFamily:'Roboto-Medium'}}> No Users Found </Text>
