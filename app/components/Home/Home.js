        ...
        var path=Config.API_URL+'/contact/create';
         ...
         ApiUtility.fetchPost(path,bundle,function(response){
              // that.fetchContacts(_id);
                ...
                if(response.success){
                  AuthUtility.setKey("isContact",'false',function(){
                    that.setState({fetchContactsList:true,userid:_id,modalVisible:true});
                          that.props.navigation.navigate('ProfessionsMultiPick',{ 
                            selectedArr:that.state.subcategory,
                            fetchContactsList:true,
                            userid:that.state.userid});
                              // alert(modalVisible+'modalVisible')
                              // that.fetchContacts(_id);
...
getAllData(loadMore,refreshList){
  ...
  // alert(this.props.listtype+' $$ '+this.props.post.isFinished)

  if(this.props.post.isFinished == undefined)
    return;
...
deletePost(id){
  ...
  [
    {text: 'Remove', onPress: () => {that.props.postAction.fetchPostDelete(path,bundle,this.state._id);
                                      }
    },
...
onListFooterComponent(){
  ...
  if((this.props.post.postList || this.props.post.filterpostList)  && (isFinished || this.props.post.isFinished == undefined))
  {
