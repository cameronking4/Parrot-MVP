...
componentWillMount(){
  ...
  const selectedArr = navigation.getParam('selectedArr', null);
  const userid = navigation.getParam('userid', null);
  const fetchContactsList = navigation.getParam('fetchContactsList', null);
  ...
  AuthUtility.getToken(function(token){
      ...
      that.setState({
        ...
        categoryCheckedArray : selectedArr,
        userid:userid,
        fetchContactsList:fetchContactsList
      },that.refreshCheckArrFromCheckArray(selectedArr));
       // console.log("Home: getToken: ",token); 
  });

  AuthUtility.getUserField("_id",function(id){
          that.setState({loginId:id},()=>{that.fetchData('')});
  });
...
componentDidMount(){
  // alert()
  if(this.state.fetchContactsList){
    this.fetchData('');  
  }
  ...
  // alert(this.props.userid);
   var path=Config.API_URL+'/contact/listing';
         ...
         ApiUtility.fetchPost(path,bundle,function(response){
          ...
          if(response)
          {
              that.setState({contactListArray:response.data.data},()=>{that.dataProcess(response.data.data);})
...
createMatch(){
  ...
  var that =this,i=0,tmpArr=[];

  var categoryCheckedArray = this.state.categoryCheckedArray;
  
  // alert(categoryCheckedArray.length);

  for(i=0;i<categoryCheckedArray.length;i++){
    tmpArr.push(categoryCheckedArray[i]._id);
  }
  // alert(JSON.stringify(tmpArr))
  
  var path=Config.API_URL+'/match/unique-create-many';
           
        var bundle={
            matchforusers:tmpArr,
        }; 
 ...
 <View style={{flexDirection: 'column',alignItems:"center",justifyContent:"center",marginTop:Platform.OS=='ios'?20:0,height:Platform.OS=='ios'?Screen.height:Screen.height,paddingTop:Platform.OS=='ios'?10:0,}} >
        ...
        <View style={{height:Screen.height,marginTop:20,width:Screen.width,}}>
          <View style={{height:40,backgroundColor:color.text3,flexDirection:"row",justifyContent:"space-between"}}>
                <TouchableOpacity  style={{height:40,justifyContent:"center",marginLeft:5}} onPress={()=>{this.props.navigation.goBack(null)}}>
                  ...
                  <Text style={{textAlign:"center",color:"#FFFFFF",fontSize:20,}}>{"Select your Friends"}</Text>
                   ...
                   <TouchableOpacity style={{height:40,justifyContent:"center",marginRight:5}} onPress={()=>{this.createMatch()}}>
                      <View>
